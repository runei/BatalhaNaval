...
.data 
	...
	STR_MSG_VAZIA DB "$"
	STR_ACERTOU_ALGO DB "Acertou algo!$"
	STR_ERROU_TIRO DB "Errou!$"
	STR_VOCE_VENCEU DB "Voce venceu!$"
	STR_SUA_VEZ DB "Sua vez!$"
	STR_MSG_ATUAL DB OFFSET STR_SUA_VEZ
...
.code
	...
	ATIRA_MATRIZ_NAVIOS PROC				; recebe posicao em AX, retorna em DX se acertou
		...
		MOV DX, ERROU
		MOV BX, OFFSET STR_MSG_ATUAL
		MOV BYTE PTR [BX], OFFSET STR_ERROU_TIRO
		...
		AMN_ACERTOU:
			...
			MOV BX, OFFSET STR_MSG_ATUAL
			MOV BYTE PTR [BX], OFFSET STR_ACERTOU_ALGO
	...
	ESPERAR_SEG PROC ; Recebe em AL o numero de seg
		PUSH AX
		PUSH BX
		PUSH CX
		PUSH DX
	...
	TELA_JOGO PROC
		...
		TJ_LACO:
			...
			XOR BX, BX
			MOV BL, STR_MSG_ATUAL
			MOV DX, BX
			CALL PRINTFS
			
			SETA_CURSOR_POSICAO:
				MOV DH, 16 ; Linha = colunas + linhas em branco
				MOV DL, 9  ; Posicao:
				CALL SET_CURSOR
		...
		TJ_FIM:
			MOV BX, OFFSET STR_MSG_ATUAL
			MOV BYTE PTR [BX], OFFSET STR_VOCE_VENCEU
			XOR BX, BX
			MOV BL, STR_MSG_ATUAL
			MOV DX, BX
			CALL PRINTFS
